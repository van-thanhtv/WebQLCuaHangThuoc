...
<form class=" row mt-3 ms-0 pe-0" action="/Drug/update?id=${drug.id}" method="post">
    ...
    <div class="mb-3 col-6">
        ...
        <select class="form-select" name="typeDrugID" >
            <c:forEach items="${ dsTypeDrug }" var="type">
                <option ${drug.idType.id==type.id ? "selected" : ""} value="${ type.id }">
                        ${ type.name }
                </option>
            </c:forEach>
        </select>
    ...
    <div class="mb-3 col-6">
        ...
        <select class="form-select" name="formDrugID" >
            <c:forEach items="${ dsFormDrug }" var="form">
                <option ${drug.idForm.id==form.id ? "selected" : ""} value="${ form.id }">
                        ${ form.name }
                </option>
            </c:forEach>
        </select>
    ...
    <div class="mb-3 col-6">
        ...
        <input type="tel" class="form-control" name="term" value="${drug.term}">
    </div>
    <div class="mt-3">
